...
uintptr_t wnbios_lib::attach(const char* image_name)
{
...
uint64_t wnbios_lib::generate_virtual_address(uint64_t pml4, uint64_t pdpt, uint64_t offset)
{
...
PAGE_DATA wnbios_lib::valid_pml4e(uintptr_t dtb)
{
	// find a valid entry
	for (int i = 1; i < 256; i++)
	{
		...
		if (!read_physical_memory(dtb + (i * sizeof(uintptr_t)), &pml4e, sizeof(pml4e)))
		{
			return PAGE_DATA{};
		}
...
PAGE_DATA wnbios_lib::free_pdpte(uintptr_t pdpt)
{
	// find a free entry
	for (int i = 0; i < 512; i++)
	{
		...
		if (!read_physical_memory(pdpt + (i * sizeof(uintptr_t)), &pdpte, sizeof(pdpte)))
		{
			return PAGE_DATA{};
		}
...
uintptr_t wnbios_lib::insert_custom_pdpte(uint64_t pfn)
{
	// find a free pte and populate other indices while at it
	PAGE_DATA pml4e_data = valid_pml4e(cr3);
	PAGE_DATA pdpte_data = free_pdpte(pml4e_data.pfn * 0x1000);
	...
	write_physical_memory(pdpte_phys, &pdpte, sizeof(PDPTE));
